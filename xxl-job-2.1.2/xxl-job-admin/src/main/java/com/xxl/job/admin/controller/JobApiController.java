...
public class JobApiController {
    ...
    @ResponseBody
    @PermissionLimit(limit = false)
    public ReturnT<String> addStart(HttpServletRequest request, @RequestBody(required = false) String data) {
        validAccessToken(request);
        ReturnT<String> addRe = xxlJobService.add((XxlJobInfo) parseParam(data, XxlJobInfo.class));
        if (addRe.getCode() != ReturnT.SUCCESS_CODE) {
            return addRe;
        }
        Integer jobId = Integer.valueOf(addRe.getContent());
        ReturnT<String> startRe = xxlJobService.start(jobId);
        if(startRe.getCode()!=ReturnT.SUCCESS_CODE){
            return startRe;
        }
    ...
    @ResponseBody
    @PermissionLimit(limit = false)
    public ReturnT<String> remove(HttpServletRequest request,@RequestBody(required = false) String data) {
        validAccessToken(request);
        return xxlJobService.remove(Integer.valueOf(data));
    }
    // ---------------------- job biz ----------------------
